...
const UINT TAG_ENTRANCES_LISTBOX = 998;
const UINT TAG_DELETE = 997;
const UINT TAG_DONE = 996;
...
//Populate list box with all of a certain data type in hold.
{
	...
	bool bShowButtons[3] = {true, false, false};
	switch (datatype)
	{
		default:
		case Entrances:
	...
	switch (datatype)
	{
		case Entrances:
		{
			...
			this->pListBoxWidget->AddItem(END_HOLD_LEVEL_ID, g_pTheDB->GetMessageText(MID_DefaultExit));
		...
		case WorldMaps:
		{
			...
			{
				//Represent world map IDs with converted IDs for uniqueness from level entrance IDs.
				const HoldWorldMap& map = *(*sorted);
				this->pListBoxWidget->AddItem(
					LevelExit::ConvertWorldMapID(map.worldMapID), map.nameText.c_str());
			}
		}
		break;
		...
		case Speech:
		{
			...
			for (UINT wIndex=0; wIndex<speech.size(); ++wIndex)
			{
				...
				else if (dwSpeakerTextID == MID_Custom)
				{
					...
					const CMonster *pMonster = this->pCurrentGame->pRoom->GetMonsterAtSquare(
					...
					if (pMonster)
					{
						...
						if (pMonster->wType == M_CHARACTER) {
							...
							customIdentity = pCharacter->wLogicalIdentity;
						}
