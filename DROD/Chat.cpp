...
//Wait for any outstanding queries/sends to finish before exiting.
{
	if (this->handle)
	{
		...
		CNetResult* pBuffer = g_pTheNet->GetResults(this->handle);
		delete pBuffer;
		this->handle = 0;
	}
}
...
//Returns: true if operations completed normally; false if server response error occurred
{
	...
	if (this->handle)
	{
		if (g_pTheNet->GetStatus(this->handle) >= 0)
		{
			...
			{
				bBadDataReceived = !Parse(pBuffer->pJson);
				delete pBuffer;
...
//*****************************************************************************
bool Interface::Parse(Json::Value *pChat)
//Parses CaravelNet chat updates.
//
//Returns: true if data received were valid, else false
{
	if (!pChat->isMember("chats") || !pChat->isMember("active")) return false;
	const Json::Value texts = (*pChat)["chats"];
	const Json::Value users = (*pChat)["active"];
	...
	for (unsigned int i = 0; i < texts.size(); i++) {
		...
		Data *c = new Data;
		c->chatID = text.get("ID", 0).asUInt();
		if (this->chatIDsReceived.has(c->chatID)) {
			delete c;
		} else {
			c->bAFK = false;
			if (text.isMember("Admin")) {
				c->bAdminMessage = true;
				Base64::decode(text["Admin"].asString(), c->text);
			} else {
				c->bAdminMessage = false;
			}

			if (text.isMember("Text")) {
				Base64::decode(text["Text"].asString(), c->text);
			}
			...
			if (text.isMember("RefreshFreq")) {
				SetRefreshInterval(text["RefreshFreq"].asUInt());
			}
			if (text.isMember("Color")) {
				const Json::Value& color = text["Color"];
				c->color.r = color["r"].asUInt();
				c->color.g = color["g"].asUInt();
				c->color.b = color["b"].asUInt();
			}
