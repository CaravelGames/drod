...
//*****************************************************************************
void CDbLevels::GetRequiredRooms(
	UINT levelID,
	CIDSet& requiredRooms) //(out)
{
	requiredRooms.clear();

	CIDSet roomsInLevel = CDb::getRoomsInLevel(levelID);
	...
	const bool bConquered)                         //only conquered rooms [default=false]
{
	...
	//Total rooms explored/conquered.
			...
			iter != playerSavedGamesInLevel.end(); ++iter)
	{
		...
		if (bConquered)
		{
			...
			pSavedGame->ExploredRooms.intersect(roomsInLevel);
			playerRoomsExploredInLevel += pSavedGame->ExploredRooms;
	...
	UINT &dwRoomX, UINT &dwRoomY)  //(out)  The starting room coords.
{
	...
	if (dwRoomID) {
		c4_View RoomsView;
		const UINT dwRoomI = CDbBase::LookupRowByPrimaryKey(dwRoomID, V_Rooms, RoomsView);
...
//Params:
	CIDSet& requiredRooms)  //(out)
{
	CDbLevels::GetRequiredRooms(this->dwLevelID, requiredRooms);
}
...
//Tallies the value of stat-affecting items in the level.
{
	...
	CIDSet roomIDs = CDb::getRoomsInLevel(this->dwLevelID);
	for (CIDSet::const_iterator room = roomIDs.begin(); room != roomIDs.end(); ++room)
	{
		const CDbRoom *pRoom = g_pTheDB->Rooms.GetByID(*room);
		ASSERT(pRoom);
	...
	bool &bSaveRecord)      //(out) whether record should be saved
{
	switch (pType)
	{
		case P_LevelID:
		{
			...
			while (true)
			{
				...
				if (info.typeBeingImported != CImportInfo::Hold)
				{
					...
					PrimaryKeyMap::const_iterator localID;
			...
			if (dwLocalLevelID)
			{
				...
				if (info.typeBeingImported == CImportInfo::LanguageMod)
				{
					...
					//Load the level room object into this record.
					CDbLevel *pLocalLevel = g_pTheDB->Levels.GetByID(dwLocalLevelID);
					ASSERT(pLocalLevel);
					SetMembers(*pLocalLevel, true);
					delete pLocalLevel;
				}
			} else {
		...
		{
			ASSERT(!info.pEntrance);
			info.pEntrance = new CEntranceData();
			info.pEntrance->bIsMainEntrance = true;  //all 1.6 entrances are unique to a level
			info.pEntrance->eShowDescription = CEntranceData::DD_Always; //all 1.6 entrance texts are shown
			WSTRING data;
			Base64::decode(str,data);
			info.pEntrance->DescriptionText = data.c_str();
			break;
		}
		case P_RoomID:
			info.pEntrance->dwRoomID = convertToUINT(str);
			//Room hasn't been read in yet and local ID must be set later
			break;
		case P_X:
			info.pEntrance->wX = convertToUINT(str);
			break;
		case P_Y:
			info.pEntrance->wY = convertToUINT(str);
			break;
		case P_O:
			info.pEntrance->wO = convertToUINT(str);
			//Done gathering entrance data.
			info.LevelEntrances.push_back(info.pEntrance);
			info.pEntrance = NULL;
			break;
