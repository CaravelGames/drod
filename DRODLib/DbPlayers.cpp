									...
									//       (default=true)
{
	...
	if (bRetainRef)
	{
		...
		db.Demos.FilterByShow(0, true);
		db.Demos.GetIDs(DemoShowIDs);
		db.Demos.FilterByShow(0, false);
	}
...
//*****************************************************************************
void ExportXMLPlayerChallenges(
	const Challenges& challenges,
	string& str) //(in/out)
{
	UINT dwBufSize;
	CDbPackedVars packedChallenges;
	challenges.serialize(packedChallenges);
	BYTE *pBuf = packedChallenges.GetPackedBuffer(dwBufSize);
	if (dwBufSize > 4)
	{
		str += PROPTAG(P_Challenges);
		str += Base64::encode(pBuf,dwBufSize-4);   //remove null UINT
	}
	delete[] pBuf;
}
	...
	const bool bRef)        //(in) Only export GUID reference (default = false)
{
	...
	//Skip bIsImported flag when exporting a player.  It will always be set on import.
	...
	if (bRef)
	{
		...
		switch (dbRefs.eSaveType)
		{
			...
			case ST_EndHold:
			{
				...
				const UINT dwEndHoldID = g_pTheDB->SavedGames.FindByEndHold(g_pTheDB->GetHoldID(), dwPlayerID);
				if (dwEndHoldID)
			...
			{
				//Only the hold mastered saved game for the current hold, if any, should be exported.
				str += CLOSETAG;

				const UINT savedGameID = g_pTheDB->SavedGames.FindByHoldMastered(g_pTheDB->GetHoldID(), dwPlayerID);
				if (savedGameID)
					db.SavedGames.ExportXML(savedGameID, dbRefs, str);
			}
			break;
			case ST_Progress:
			case ST_ProgressEndHold:
			{
				...
				const UINT dwProgressID = g_pTheDB->SavedGames.FindByType((SAVETYPE)dbRefs.eSaveType);
				if (dwProgressID)
					db.SavedGames.ExportXML(dwProgressID, dbRefs, str);
			}
			break;
			case ST_Cloud:
				//Minimal player data, with no saves included.
				str += CLOSETAG;
			break;
			default:
			{
				//These values are invalid outside the local installation.
				pPlayer->Settings.Unset(Settings::CloudActivated);
				pPlayer->Settings.Unset(Settings::CloudHoldVersion);
				pPlayer->Settings.Unset(Settings::CloudHoldDemosVersion);
				...
				str.reserve(str.capacity() + static_cast<UINT>(fNumDemos*1000)
						+ static_cast<UINT>((fNumSavedGames-fNumDemos)*500)); //large speed optimization
				...
				{
					CDbXML::PerformCallbackf(0.4f + (++wCount/fNumSavedGames) * 0.6f);
					if (CDbXML::info.bQuickPlayerExport)
					{
...
//True if successful, false if not.
{
	...
	ASSERT(IsOpen());
	c4_View PlayersView;
	const UINT dwPlayerI = LookupRowByPrimaryKey(dwLoadPlayerID, V_Players,
			PlayersView);
	...
	this->bIsImported = ((UINT) p_IsImported(row) != 0);
...
//*****************************************************************************
Challenges CDbPlayer::RekeyImportedChallenges(
	const string& str,
	CImportInfo &info) //(in)
{
	BYTE *data;
	Base64::decode(str,data);
	CDbPackedVars packedChallenges((const BYTE*)data);
	delete[] data;

	Challenges challenges(packedChallenges);

	//Remap holdIDs to local values.
	const set<UINT> holdIDs = challenges.getHoldIDs();
	map<UINT,UINT> holdIDmap;
	for (set<UINT>::const_iterator it=holdIDs.begin(); it!=holdIDs.end(); ++it)
	{
		const UINT oldHoldID = *it;
		PrimaryKeyMap::const_iterator localHoldID = info.HoldIDMap.find(oldHoldID);
		if (localHoldID == info.HoldIDMap.end())
			continue;

		const UINT newHoldID = localHoldID->second;
		holdIDmap.insert(make_pair(oldHoldID, newHoldID));

#ifdef STEAMBUILD
		if (CDbHold::IsOfficialHold(CDbHolds::GetStatus(newHoldID))) {
			//Submit steam achievements for all completed challenges.
			const WSTRING holdName = CDbHolds::GetHoldName(newHoldID);
			if (!holdName.empty()) {
				const set<WSTRING> completed = challenges.get(oldHoldID);
				for (set<WSTRING>::const_iterator chIt=completed.begin(); chIt!=completed.end(); ++chIt)
					CDb::SubmitSteamAchievement(holdName, UnicodeToUTF8(*chIt));
			}
		}
#endif
	}
	challenges.rekeyHoldIDs(holdIDmap);
	...
	bool &bSaveRecord)      //(out) whether record should be saved
{
	switch (pType)
	{
		case P_PlayerID:
		{
			...
			//If the first player record being imported is only a reference,
			...
			if (!localPlayerIDs.empty())
			{
				...
				if (this->bIsLocal)
				{
					...
					this->bIsImported = true; //player record is being imported
					...
					if (bOldPlayerIsLocal)
					{
						...
						CDbPackedVars packedChallenges;
						packedChallenges = p_Challenges(row);
						const Challenges oldChallenges(packedChallenges);
						this->challenges += oldChallenges;

						//Remove old player from the DB.  This new one replaces it.
						g_pTheDB->Players.Delete(dwLocalPlayerID);
					} else {
						//The existing player record is non-local -- it gets overwritten by this one.
						//Don't delete the old one first, because holds could be attached to it.
					}
		...
		case P_Settings:
		{
			BYTE *data;
			Base64::decode(str,data);
			this->Settings.UseOldFormat(info.wVersion < 201);
			this->Settings = (const BYTE*)data;
			//Profiles prior to DROD4 had EyeCandy as a Boolean, not a Byte
			if (info.wVersion < 323)
			{
		...
		case P_Challenges:
			if (!bSaveRecord) {
				//When syncing player cloud saves,
				//challenges will have to be merged later, after hold key map has been parsed.
				info.playerChallenges[this->dwPlayerID] = str;
			} else {
				this->challenges = RekeyImportedChallenges(str, info);
			}
			break;
		default:
			return MID_FileCorrupted;
...
//Update existing Players record in database.
{
	...
	CDbPackedVars packedChallenges;
	this->challenges.serialize(packedChallenges);

	UINT dwChallengesSize;
	BYTE *pbytChallengesBytes = packedChallenges.GetPackedBuffer(dwChallengesSize);
	if (!pbytChallengesBytes) return false;
	c4_Bytes ChallengesBytes(pbytChallengesBytes, dwChallengesSize);

	//Update Players record.
	p_IsImported(row) = this->bIsImported;
	p_IsLocal(row) = this->bIsLocal;
	p_LastUpdated(row) = UINT((time_t)this->LastUpdated);
	p_Settings(row) = SettingsBytes;
	p_Challenges(row) = ChallengesBytes;
