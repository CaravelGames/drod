...
//sorted by room and level.
{
	...
	holdText += pHold->NameText;
	holdText += endLine;

	holdText += pHold->DescriptionText;
	holdText += endLine;

	holdText += pHold->EndHoldText;
	holdText += endLine;
			...
			character != pHold->characters.end(); ++character)
	{
		...
		if (scriptIDs.contains(c.dwScriptID))
		{
			...
			wNextFreeScriptID = scriptIDs.getMax() + 1;
			c.dwScriptID = wNextFreeScriptID;
			c.ExtraVars.SetVar(scriptIDstr, c.dwScriptID);
			bScriptIDRepaired = true;
		}
		VERIFY(!(scriptIDs += c.dwScriptID).empty());
	}
	...
	for (SORTED_LEVELS::const_iterator level = levels.begin(); level != levels.end(); ++level)
	{
		...
		roomIDs = CDb::getRoomsInLevel(pLevel->dwLevelID);

		//Output level entrance texts.
		holdText += wszHeader;
		holdText += pLevel->NameText;
		holdText += wszEndHeader;
		holdText += endLine;
		...
		for (UINT wIndex=0; wIndex<pHold->Entrances.size(); ++wIndex)
		{
			...
			if (roomIDs.has(pEntrance->dwRoomID))
			{
				...
				holdText += wszColon;
				holdText += wszSpace;
				holdText += pEntrance->DescriptionText;
				holdText += endLine;
			}
		}
	...
	const bool bRef)        //(in) Only export GUID reference (default = false)
{
	...
	else
	{
		...
		for (dwIndex=0; dwIndex<dwSize; ++dwIndex) 
		{
			...
			if (entrance.dwDataID)
				g_pTheDB->Data.ExportXML(entrance.dwDataID, dbRefs, str);

			WSTRING const wMessage = (WSTRING)entrance.DescriptionText;

			str += STARTVPTAG(VP_Entrances, P_EntranceID);
			str += INT32TOSTR(entrance.dwEntranceID);
			str += PROPTAG(P_DescriptionMessage);
			str += Base64::encode(wMessage);
			str += PROPTAG(P_RoomID);
			str += INT32TOSTR(entrance.dwRoomID);
			str += PROPTAG(P_X);
			str += INT32TOSTR(entrance.wX);
			str += PROPTAG(P_Y);
			str += INT32TOSTR(entrance.wY);
			str += PROPTAG(P_O);
			str += INT32TOSTR(entrance.wO);
			str += PROPTAG(P_IsMainEntrance);
			str += INT32TOSTR(entrance.bIsMainEntrance);
			str += PROPTAG(P_ShowDescription);
			str += INT32TOSTR(entrance.eShowDescription);
			//Only save data if it's not a dangling reference.
			if (g_pTheDB->Data.Exists(entrance.dwDataID))
			{
				str += PROPTAG(P_DataID);
				str += INT32TOSTR(entrance.dwDataID);
			}
			str += CLOSETAG;
		}
		...
		CDbXML::PerformCallbackf(fBasePercentDone + (wCount/fItems) * fTotalRemainingPercent);
		db.Demos.FilterByShow(dwHoldID);
...
const
{
	...
	for (UINT wIndex=pHold->Entrances.size(); wIndex--; )
	{
		CEntranceData &entrance = *(pHold->Entrances[wIndex]);
		if (dwRoomID == entrance.dwRoomID)
		{
			CEntranceData *pEntranceCopy = new CEntranceData();
			pEntranceCopy->SetMembers(entrance, false); //replicates data in DB
...
//*****************************************************************************
WSTRING CDbHolds::GetHoldName(const UINT holdID)
//Returns: the name of the hold with specified ID
{
	WSTRING name;
	CDbHold *pHold = GetByID(holdID, true);
	if (pHold)
	{
...
//OUT: set of room IDs explored/conquered by player in specified hold
//
//Params:
	const UINT dwHoldID, const UINT dwPlayerID,   //(in)
	CIDSet& rooms, //(out)
	const bool bOnlyConquered) //[default=false]
{
	ASSERT(dwHoldID);
	ASSERT(dwPlayerID);
	...
	for (CIDSet::const_iterator iter = savedGameIDs.begin(); iter != savedGameIDs.end(); ++iter)
	{
		...
		if (type != ST_DemoUpload && type != ST_Unknown) //ignore non-real saved game records
		{
			rooms += bOnlyConquered ? db.SavedGames.GetConqueredRooms(*iter) :
					db.SavedGames.GetExploredRooms(*iter);
		}
	}

	//Total rooms explored/conquered for this hold.
	const UINT savedGameID = db.SavedGames.FindByType(ST_PlayerTotal, dwPlayerID, false);
	if (savedGameID)
	{
		CIDSet roomsInHold = CDb::getRoomsInHold(dwHoldID);
		if (bOnlyConquered)
		{
			CIDSet roomIDs = db.SavedGames.GetConqueredRooms(savedGameID);
			roomIDs.intersect(roomsInHold);
			rooms += roomIDs;
		} else {
			CIDSet roomIDs = db.SavedGames.GetExploredRooms(savedGameID);
			roomIDs.intersect(roomsInHold);
			rooms += roomIDs;
		}
	}
	...
	CIDSet& exploredRooms, CIDSet& conqueredRooms) //(out)
{
	...
	for (CIDSet::const_iterator iter = savedGameIDs.begin(); iter != savedGameIDs.end(); ++iter)
	{
		...
		if (type != ST_DemoUpload && type != ST_Unknown) //ignore non-real saved game records
		{
			conqueredRooms += db.SavedGames.GetConqueredRooms(*iter);
			exploredRooms += db.SavedGames.GetExploredRooms(*iter);
		}
	}
	...
	if (savedGameID)
	{
		...
		CIDSet roomIDs = db.SavedGames.GetConqueredRooms(savedGameID);
		roomIDs.intersect(roomsInHold);
		conqueredRooms += roomIDs;

		roomIDs = db.SavedGames.GetExploredRooms(savedGameID);
		roomIDs.intersect(roomsInHold);
		exploredRooms += roomIDs;
	}
}
...
const
{
	//Get rooms and secret rooms in hold.
	GetRooms(dwHoldID, stats);
	const UINT numSecretRooms = stats.secretRooms.size();
	...
	if (savedGameID)
	{
		if (bConqueredOnly)
		{
			roomsExplored = db.SavedGames.GetConqueredRooms(savedGameID);
		} else {
			roomsExplored = db.SavedGames.GetExploredRooms(savedGameID);
		}
		roomsExplored.intersect(stats.secretRooms); //filter by secret rooms in hold
	...
	for (CIDSet::const_iterator iter = savedGameIDs.begin(); iter != savedGameIDs.end(); ++iter)
	{
		...
		if (type != ST_DemoUpload && type != ST_Unknown) //ignore non-real saved game records
		{
			CIDSet roomIDs = bConqueredOnly ? db.SavedGames.GetConqueredRooms(*iter) :
					db.SavedGames.GetExploredRooms(*iter);
			roomIDs.intersect(stats.secretRooms);
			roomsExplored += roomIDs;
			if (roomsExplored.size() == numSecretRooms)
				return numSecretRooms;
		}
	}

	ASSERT(roomsExplored.size() <= numSecretRooms);
	return roomsExplored.size();
}
...
//*****************************************************************************
bool CDbHolds::IsHoldCompleted(const UINT dwHoldID, const UINT playerID)
{
	return g_pTheDB->SavedGames.FindByEndHold(dwHoldID, playerID) != 0;
}

//*****************************************************************************
bool CDbHolds::IsHoldMastered(const UINT dwHoldID, const UINT playerID)
//Returns: true if specified player has ended the hold, plus conquered all the secret rooms.
{
	return g_pTheDB->SavedGames.FindByHoldMastered(dwHoldID, playerID) != 0;
}

//*****************************************************************************
bool CDbHolds::ScanForNewHoldMastery(
	const UINT dwHoldID,
	UINT playerID, //[default=0 (current)]
	const UINT ignoreMasterySaves) //[default=false]
const
{
	if (!playerID)
	...
	if (!ignoreMasterySaves && IsHoldMastered(dwHoldID, playerID))
		return true; //if already mastered, don't perform the expensive secret room check below

	if (!IsHoldCompleted(dwHoldID, playerID))
		return false;
...
//*****************************************************************************
void CDbHolds::LogScriptVarRefs(const UINT holdID, const bool bChallenges)
//Output a log of all vars used in the hold.
{
	...
	VARCOORDMAP varMap;
	const CIDSet roomIDs = GetScriptCommandRefs(pHold, bChallenges, varMap);
	for (CIDSet::const_iterator roomIt=roomIDs.begin(); roomIt!=roomIDs.end(); ++roomIt)
		GetScriptCommandRefsForRoom(*roomIt, pHold, bChallenges, varMap);
	delete pHold;
...
//*****************************************************************************
void CDbHolds::GetScriptChallengeRefs(
	const CDbHold *pHold,
...
void CDbHolds::CheckForVarRefs(
	const CCharacterCommand& c, const bool bChallenges,
...
//Makes a copy of all Data and Speech records marked as owned by this hold and accessed
//in some record owned by the hold, and gives ownership of the copies to the new hold.
{
	...
	//Copy hold entrance media.
	for (wIndex=0; wIndex<pNewHold->Entrances.size(); ++wIndex)
	{
		CEntranceData& entrance = *(pNewHold->Entrances[wIndex]);
		CDbData::CopyObject(info, entrance.dwDataID, newHoldID);
	}

	for (wIndex=0; wIndex<pNewHold->worldMaps.size(); ++wIndex)
	{
		HoldWorldMap& map = pNewHold->worldMaps[wIndex];
		CDbData::CopyObject(info, map.dataID, newHoldID);
	}
...
//The caller must remember to Update() the hold if this character gets used.
{
	...
	this->characters.push_back(new HoldCharacter(dwNewCharID, pwszName, dwNewScriptID));
	return dwNewCharID;
}
...
{
	ASSERT(IsVarNameGoodSyntax(pwszName));
...
//Returns: whether a var name has correct syntax
{
	...
	while (WCv(*pName))
	{
		if (!(IsVarCharValid(*pName)))
			return false;
		++pName;
	}
	if (pName[-1] == W_t(' ')) //last char cannot be a space
		return false;
	return true;
}
...
//NOTE: Main entrances should only be deleted through calls by DeleteEntrancesForRoom().
{
	...
	//Retain pEntrance->dwDataID in the DB, in case there are other references
	delete pEntrance;
...
//*****************************************************************************
UINT CDbHold::getRoleForLogicalIdentity(const UINT logicalIdentity) const
{
	UINT role = logicalIdentity;

	const HoldCharacter* pCustomChar = GetCharacterConst(logicalIdentity);
	if (pCustomChar)
		role = pCustomChar->wType;
...
//Returns: whether specified player may edit this hold.
{
	...
	switch (editAccess)
	{
		...
		case CDbHold::YouAndConquerors:
			return g_pTheDB->Holds.IsHoldCompleted(dwHoldID, playerID);
			...
			room != roomsInHold.end(); ++room)
	{
		...
		for (UINT wExitI = 0; wExitI < wExitCount; ++wExitI)
		{
			...
			if (dwEntranceID)
			{
				...
				if (!pEntrance)
				{
					const bool bWorldMap = LevelExit::IsWorldMapID(dwEntranceID) &&
							DoesWorldMapExist(LevelExit::ConvertWorldMapID(dwEntranceID));
					if (!bWorldMap)
	...
	c4_View &CharactersView)    //(in) Open view containing 0 or more characters
{
	...
	for (UINT wCharI=0; wCharI<wSize; ++wCharI)
	{
		...
		CDbPackedVars ExtraVars;
		ExtraVars = p_ExtraVars(row);
		UINT dwScriptID = ExtraVars.GetVar(scriptIDstr, 0);
...
//Returns: entrance ID of main entrance in specified level, 0 if none
{
	...
	{
		CEntranceData &entrance = *(this->Entrances[wIndex]);
		if (entrance.bIsMainEntrance)
		{
			const UINT dwRoomID = entrance.dwRoomID;
			if (dwLevelID == g_pTheDB->Rooms.GetLevelIDForRoom(dwRoomID))
				return entrance.dwEntranceID;
		}
	}
	return 0;
}
...
//*****************************************************************************
WSTRING CDbHold::GetLocalScriptVarNameForID(UINT varID) const
{
	map<UINT, WSTRING>::const_iterator it = this->localScriptVars.find(varID);
...
char* CDbHold::getVarAccessToken(const char* pName) const
{
	ASSERT(pName);
	WSTRING wstr;
...
//*****************************************************************************
UINT CDbHold::VarIsSetInAnySavedGame(const char* pszName, const UINT playerID) const
{
	if (!pszName)
		return 0;

	//Get all player's saved games in this hold.
	CDb db;
	db.SavedGames.FilterByHold(this->dwHoldID);
	db.SavedGames.FilterByPlayer(playerID);
	db.SavedGames.FindHiddens(true);
	CIDSet ids = db.SavedGames.GetIDs();

	//Get var lookup.
	char *pVar = getVarAccessToken(pszName);

	//Scan the saved hold var values in each saved game.
	ASSERT(IsOpen());
	c4_View SavedGamesView;
	for (CIDSet::const_iterator iter = ids.begin(); iter != ids.end(); ++iter)
	{
		const UINT dwSavedGameI = LookupRowByPrimaryKey(*iter,
		...
		CDbPackedVars stats;
		stats = p_Stats(row);
		const UINT val = stats.GetVar(pVar, UINT(0));
		if (val)
			return val; //variable is set to this value in this saved game
	}

	return 0; //var not set to non-zero value anywhere
}
...
{
	ASSERT(IsVarNameGoodSyntax(newName.c_str()));
...
//Returns: pointer to new hold
{
	...
	for (UINT wIndex=pNewHold->Entrances.size(); wIndex--; )
	{
	...
	CDb db;
	db.Demos.FilterByShow(this->dwHoldID);
	const CIDSet showSequenceDemoIDs = db.Demos.GetIDs();
			...
			setIter != savedGamesInHold.end(); ++setIter)
	{
		...
		if (IsCopyableSavedGame(pSavedGame->eType))
		{
			...
			rooms.clear();
			for (room = pSavedGame->ExploredRooms.begin();
			...
			pSavedGame->ExploredRooms = rooms;
...
//Returns: whether bad data were corrected
{
	...
	if (levelIDs.has(this->dwLevelID))
		return false; //first level is valid -- no repair needed
	...
	return lowestLevelIndex < UINT(-1);
}
	...
	bool &bSaveRecord)      //(out) whether record should be saved
{
	switch (pType)
	{
		case P_HoldID:
		{
			...
			const HoldStatus eMatchHoldsWithStatus = info.typeBeingImported ==
					CImportInfo::Player ? TCB : NoStatus;	//match player imports for demo and full TCB hold
			...
			if (!dwLocalHoldID)
			{
				...
				c4_View HoldsView;
				const UINT dwHoldI = LookupRowByPrimaryKey(
						dwLocalHoldID, V_Holds, HoldsView);
				...
				case CImportInfo::Player:
							...
							info.ImportStatus == MID_ImportSuccessful)   //player record is actually being imported
					{
						...
						//Saved games in this hold must be re-verified for integrity.
...
#if defined(STEAMBUILD) && !defined(DEV_BUILD) && !defined(DEBUG)
			if ((IsOfficialHold(this->status) || this->status == CDbHold::Official) &&
	...
	CImportInfo &info)  //(in/out) Import data
{
	...
	{
		case VP_Entrances:
			ASSERT(pType == P_Start || info.pImportEntrance);
			switch (pType)
			{
				case P_Start:
					ASSERT(!info.pImportEntrance);
				...
				case P_ShowDescription:
					...
					break;
				case P_DataID:
					info.pImportEntrance->dwDataID = convertToUINT(str);
					if (info.pImportEntrance->dwDataID)
					{
						//Set to local ID.
						PrimaryKeyMap::const_iterator localID = info.DataIDMap.find(info.pImportEntrance->dwDataID);
						if (localID == info.DataIDMap.end())
							return MID_FileCorrupted;  //record should have been loaded already
						info.pImportEntrance->dwDataID = localID->second;
					}
					break;
				case P_End:
					//Finish processing
		...
		case VP_Characters:
			switch (pType)
			{
				...
				case P_DataID:
					...
					{
						//Set to local ID.
						PrimaryKeyMap::const_iterator localID = info.DataIDMap.find(info.importChar.dwDataID_Avatar);
						if (localID == info.DataIDMap.end())
							return MID_FileCorrupted;  //record should have been loaded already
				...
				case P_DataIDTiles:
					...
					{
						//Set to local ID.
						PrimaryKeyMap::const_iterator localID = info.DataIDMap.find(info.importChar.dwDataID_Tiles);
						if (localID == info.DataIDMap.end())
							return MID_FileCorrupted;  //record should have been loaded already
				...
				case P_ExtraVars:
				{
					...
					delete[] data;
					const MESSAGE_ID val = pCharacter->ImportSpeech(info, true);
					if (val != MID_ImportSuccessful)
						return val;

					info.importChar.ExtraVars = pCharacter->ExtraVars;
					UINT dwScriptID = info.importChar.ExtraVars.GetVar(scriptIDstr, UINT(0));
					info.importChar.dwScriptID = dwScriptID;
					delete pCharacter;
	...
	c4_View &EntrancesView)    //(in) Open view to fill.
{
	...
	for (wEntranceI=0; wEntranceI < wSize; ++wEntranceI)
	{
		...
		p_DataID(row) = entrance.dwDataID;
	}
...
//Frees resources associated with this object and resets member vars.
{
	...
	this->dwNewLevelIndex = this->dwScriptID = this->dwVarID =
		this->dwCharID = this->dwWorldMapID = 0;
