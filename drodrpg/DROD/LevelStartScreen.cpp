// $Id: LevelStartScreen.cpp 8370 2007-10-20 16:33:59Z mrimer $
...
//True if activation should continue, false if not.
{
	...
	ASSERT(pCurrentGame);
	const UINT dwLevelID = pCurrentGame->pLevel->dwLevelID;
	CDbLevel *pLevel = g_pTheDB->Levels.GetByID(dwLevelID);
	CDbHold *pHold = g_pTheDB->Holds.GetByID(pLevel->dwHoldID, true);
	if (!pLevel || !pHold) {ASSERT(!"Couldn't retrieve level or hold."); return false;}
