...
//	const bool bIsPlacingDouble = this->pCurrentGame ? this->pCurrentGame->swordsman.wPlacingDoubleType != 0 : false;
	...
	// O-Layer effects need to be updated and dirtied before O-Layer is drawn to ensure lighting is correctly applied for
		...
		if (bMoving && !pCropRect)
		{
					...
					this->lightedRoomTiles.has(wCol,wRow) || this->lightedPlayerTiles.has(wCol,wRow))
			{
				...
				if (bMoving) {
					...
					AddLightInterp(pDestSurface, wCol, wRow, psL, fDark, blit.wTileImageNo, blit.nOpacity, -blit.getRaisedPixels(), pCropRect);
				}
			}
		}
	}
