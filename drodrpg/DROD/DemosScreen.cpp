// $Id: DemosScreen.cpp 9605 2011-03-09 14:59:25Z TFMurphy $
...
//Constructor.
{
	...
	static const int X_DURATION = X_AUTHOR;
...
//True if activation should continue, false if not.
{
	...
	this->bDownloadDemos = (pCurrentPlayer->Settings.GetVar(useInternetStr, false));
	...
	this->bIsAuthor = pCurrentPlayer->dwPlayerID == g_pTheDB->Holds.GetAuthorID(dwHoldID);
...
//Requests the recorded CaravelNet demos for the specified room.
{
...
#else
	bool bDownloadCriteriaMet = this->bIsAuthor;
	const UINT dwHoldID = g_pTheDB->Rooms.GetHoldIDForRoom(this->dwRoomID);
	if (!bDownloadCriteriaMet)
	{
		...
		for (CIDSet::const_iterator iter = savedGameIDs.begin(); iter != savedGameIDs.end(); ++iter)
		{
			...
			if (pSavedGame->eType != ST_DemoUpload) //not a real saved game record
				if (pSavedGame->GetExploredRooms().has(dwRoomID))
				{
					bDownloadCriteriaMet = true;
					delete pSavedGame;
					break;
				}
			delete pSavedGame;
	...
	CIDSet roomIDs(dwRoomID);
	CDbRefs dbRefs(V_Rooms, roomIDs);
	string roomRef = CDbXML::getXMLheader(NULL);
	g_pTheDB->Rooms.ExportXML(dwRoomID, dbRefs, roomRef, true);
	roomRef += CDbXML::getXMLfooter();

	this->wDemoHandle = g_pTheNet->DownloadRecords(roomRef,
...
//Downloads and imports demo selected in CaravelNet list box.
{
   ...
   while (g_pTheNet->GetStatus(handle) < 0)
	...
	CStretchyBuffer *pBuffer = g_pTheNet->GetResults(handle);
	if (!pBuffer) {
	...
	MESSAGE_ID result = CDbXML::ImportXML(*pBuffer, CImportInfo::Demo);
...
//Called between events.
{
	if (this->wDemoHandle)
	{
		...
		{
			CStretchyBuffer* pBuffer = g_pTheNet->GetResults(this->wDemoHandle);
			this->wDemoHandle = 0;	//only try to get once
			if (pBuffer) {
				CStretchyBuffer CNetDemoBuffer = *pBuffer;
				delete pBuffer;
				GetCNetDemos(CNetDemoBuffer);
			}
		} else {
	...
	const UINT dwTagNo) //(in)   Widget affected by event.
{
	switch (dwTagNo)
	{
		...
		case TAG_SHOWLEVEL:
		{
			...
			if (pCurrentPlayer)
			{
				pCurrentPlayer->Settings.SetVar("ShowDemosFromLevel",
	...
	if (this->cNetDemos.empty())
	{
		this->pCNetDemoListBoxWidget->AddItem(0,
				g_pTheDB->GetMessageText(this->bHoldPublished ? MID_NoScoresForRoom :
						this->bDownloadDemos ? MID_HoldNotPublished : MID_NotConnected), true);
	} else {
...
void DemoStartElement_cb (
	void *pObject, // (in) Pointer to caller object
	const XML_Char *name, const XML_Char **atts)
{
	((CDemosScreen*)pObject)->StartElement(name, atts);
}
...
//Adds demos featured on CaravelNet to CaravelNet demos list box.
{
	...
	if (size <= 10)	//check for invalid data
	{
		...
		XML_SetElementHandler(parser, DemoStartElement_cb, NULL);
		...
		if (XML_Parse(parser, buf, size, true) == XML_STATUS_ERROR)
		{
			...
			_snprintf(errorStr, 256,
					"Demo List Parse Error: %s at line %u:%u\n",
...
//*****************************************************************************
CDemosScreen::DEMOTagType CDemosScreen::ParseTagField(const char *str)
//Returns: enumeration corresponding to tag name
const
{
	static const char *tagStr[Demo_Tag_Count] = {
		"ID", "NumMoves", "RealTime", "Time", "User", "PlayerName", "CreatedTime"
//		, "RoomNotConquered"
	};

	for (int eTag=First_Demo_Tag; eTag<Demo_Tag_Count; ++eTag)
		if (!_stricmp(str, tagStr[eTag])) //not case sensitive
			return (DEMOTagType)eTag;

	return Unknown_Demo_Tag;
}

//*****************************************************************************
void CDemosScreen::StartElement(
//Expat callback function: Process XML start tag, and attributes.
//Parses the information for one CaravelNet demo each iteration.
//
//Params:
	const XML_Char *name, const XML_Char **atts)
{
	//Ensure the only tags handled are DROD hold data.
	if (!_stricmp(name, "DROD"))
		return;
	if (_stricmp(name, "highscore"))
		return;   //not case sensitive

	CNetDemoData *pDemo = new CNetDemoData;

	//Parse demo tag fields.
	int i;
	char *str;
	for (i = 0; atts[i]; i += 2) {
		const DEMOTagType pType = ParseTagField(atts[i]);
		if (pType == Unknown_Demo_Tag)
		{
		...
		{
			case DemoID:
				pDemo->lDemoID = convertToInt(str);
				break;
			case NumMoves:
				pDemo->wNumMoves = convertToUINT(str);
				break;
			case TimeElapsed:
				pDemo->wTimeElapsed = convertToUINT(str);
				break;
			case TimeReceived:
			...
			case PlayerName:
				...
				break;
			case Created:
				pDemo->wCreated = convertToUINT(str);
				break;
			default:
