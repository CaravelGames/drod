// $Id: HoldSelectScreen.cpp 10097 2012-04-12 15:57:08Z mrimer $
...
//Constructor
{
	...
	static const UINT CY_LBOX_HEADER = CY_BUTTON;
	...
	static const UINT CX_HOLDFILTER_LIST = 160;
	...
	static const UINT CY_AUTHOR_LABEL = 30;
	...
	static const UINT CY_DESC_LABEL = Y_POSITION_LABEL - Y_DESC_LABEL;
...
//True if activation should continue, false if not.
{
	...
	if (!pCurrentPlayer) return false;
...
//Returns: true if hold was processed, false if all holds in list have been processed
{
	...
	while (this->wProcessingHoldLine < holdListCount)
	{
		...
		HoldStats stats;
		const UINT wSecretsExplored = db.Holds.GetSecretsDone(stats,
				dwHoldID, g_pTheDB->GetPlayerID());
		wStr += wszSpace;
		wStr += wszLeftParen;
		const UINT wPercent = stats.secretRooms.size() ?
				(wSecretsExplored*100)/stats.secretRooms.size() : 100;
		WCHAR temp[16];
...
//Following a hold import, download published room styles not yet installed
{
			...
			newStyle!=importedStyles.end(); ++newStyle)
	{
		...
		MESSAGE_ID result = CDbXML::ImportXML(*pBuffer, CImportInfo::Data);
...
//Downloads and installs the selected CaravelNet hold(s).
{
	...
	for (CIDSet::const_iterator line = lineNos.begin(); line != lineNos.end(); ++line)
	{
		...
		Callback(MID_ImportingData);
		CDbXML::SetCallback(this);
		result = CDbXML::ImportXML(*pBuffer, CImportInfo::Hold);
		if (!ImportConfirm(result)) {
			delete pBuffer;
			this->pProgressWidget->Hide();
			HideStatusMessage();
	...
	const UINT dwTagNo)       //(in)   Widget that event applied to.
{
	...
	switch (dwTagNo)
	{
		...
		case TAG_DELETEHOLD:
		{
			...
			CIDSet ids = this->pHoldListBoxWidget->GetSelectedItems();
...
//Upload rating for selected hold to server.
{
	...
	SetCursor(CUR_Internet);
	ShowStatusMessage(MID_RatingHold);
	...
	switch ( ((BYTE*)*pBuffer)[0]) {
		case '1':
		{
			...
			vector<CNetMedia*>& cNetMedia = g_pTheNet->GetCNetMedia();
			const int nIndex = g_pTheNet->getIndexForName(holdName.c_str());
			CNetMedia& holdData = *(cNetMedia[nIndex]);
			char temp[16];
			sprintf(temp, "%.1f", fDifficulty);
			holdData.myDifficulty = temp;
			sprintf(temp, "%.1f", fOverall);
			holdData.myRating = temp;
...
//Sets export permissions and download availability.
{
	...
	if (nIndex >= 0)
	{
		...
		CSliderWidget *pSlider = DYN_CAST(CSliderWidget*, CWidget*, GetWidget(TAG_RATEDIFFICULTY));
		{
			...
			fRating = atof(holdData.bPlayerIsAuthor ? holdData.difficulty.c_str() : holdData.myDifficulty.c_str());
			if (fRating >= 0.0)
			{
		...
		pSlider = DYN_CAST(CSliderWidget*, CWidget*, GetWidget(TAG_RATEOVERALL));
		{
			fRating = atof(holdData.bPlayerIsAuthor ? holdData.rating.c_str() : holdData.myRating.c_str());
			if (fRating >= 0.0)
			{
				sliderIndex = BYTE((fRating * pSlider->GetNumTicks() / 10.0) - 1); //(0,10] -> enum
				bPreviouslyRated = true;
		...
		pWidget->Show(holdData.bPlayerIsAuthor);
		pWidget = GetWidget(TAG_RATEHOLD);
		pWidget->Enable(!holdData.bPlayerIsAuthor);
		...
		pWidget->Show(!bPreviouslyRated && !holdData.bPlayerIsAuthor);
	}
}
...
void CHoldSelectScreen::PopulateHoldListWithLocalHolds()
{
	...
	for (CDbHold *pHold = db.Holds.GetFirst(true); pHold != NULL; pHold = db.Holds.GetNext())
	{
		...
		//holdinfo.bConquered = holdinfo.bMastered = holdinfo.bInProgress = unknown;
		//holdinfo.version = unknown;
		delete pHold;
...
void CHoldSelectScreen::PopulateHoldListWithNonLocalCaravelNetHolds()
{
	...
	for (wIndex=0; wIndex<cNetMedia.size(); ++wIndex)
	{
		...
		if (cNetHold.wVersion >= NEXT_VERSION_NUMBER) continue; //VERSION_NUMBER

		//Show non-installed CaravelNet holds.
...
{
	const UINT dwOldHoldID = g_pTheDB->GetHoldID();
	...
	const UINT dwContinueGameID = dwHoldID ? g_pTheDB->SavedGames.FindByContinue() : 0;
	...
	if (!this->pCurrentRestoreGame)
	{
		...
		this->pCurrentRestoreGame = g_pTheDB->GetNewCurrentGame(dwHoldID, Ignored);
	}
	if (this->pCurrentRestoreGame)
	...
	g_pTheDB->SetHoldID(dwOldHoldID);
}
