// $Id: EditSelectScreen.cpp 10126 2012-04-24 05:40:08Z mrimer $
...
const UINT TAG_LEVELMULTIPLIER = 1027;
...
//	static const UINT CY_ROOM_REQUIRED_BUTTON = CY_STANDARD_OPTIONBUTTON;
	static const UINT CY_ROOM_SECRET_BUTTON = CY_STANDARD_OPTIONBUTTON;
	static const UINT CY_POSITION_LABEL = CY_ROOM_SECRET_BUTTON;
...
//	const int Y_ROOM_REQUIRED_BUTTON = Y_MINIROOM - CY_ROOM_REQUIRED_BUTTON;
	...
	const int Y_ROOM_SECRET_BUTTON = Y_MINIROOM - 2*CY_ROOM_SECRET_BUTTON;

	//Map
	const UINT CX_MAP = this->w - CX_SPACE - CX_MINIROOM - CX_SPACE * 6;
	...
	static const UINT CX_WEATHER = 130;
	...
	static const int X_HOLD_FRAME = X_MINIROOM - 70;
	...
	static const int Y_WHO_CAN_EDIT_LBOX = Y_WHO_CAN_EDIT + CY_WHO_CAN_EDIT - 5;
	...
	static const int X_LEVEL_FRAME = X_HOLD_FRAME;
...
*/
	static const int X_LEVELMULTIPLIERLABEL = X_LEVEL_AUTHOR_LABEL;
	static const int Y_LEVELMULTIPLIERLABEL = Y_LEVEL_DATE_LABEL + CY_LEVEL_DATE_LABEL + 5;
	static const UINT CX_LEVELMULTIPLIERLABEL = 120;
	static const UINT CY_LEVELMULTIPLIERLABEL = CY_STANDARD_OPTIONBUTTON;
	static const int X_MULTIPLIERTBOX = X_LEVELMULTIPLIERLABEL + CX_LEVELMULTIPLIERLABEL;
	static const int Y_MULTIPLIERTBOX = Y_LEVELMULTIPLIERLABEL;
	static const UINT CX_MULTIPLIERTBOX = 75;
	static const UINT CY_MULTIPLIERTBOX = CY_STANDARD_TBOX;
	...
	pLevelSettingsFrame->AddWidget(new CLabelWidget(TAG_LEVEL_DATE_LABEL,
			...
			wszEmpty));
/*
	pOpButton = new COptionButtonWidget(TAG_SHOWEXITLEVEL,
...
*/
	pLevelSettingsFrame->AddWidget(new CLabelWidget(0,
			X_LEVELMULTIPLIERLABEL, Y_LEVELMULTIPLIERLABEL, CX_LEVELMULTIPLIERLABEL, CY_LEVELMULTIPLIERLABEL, F_Small,
			g_pTheDB->GetMessageText(MID_LevelMultiplier)));
	this->pLevelMultiplier = new CTextBoxWidget(TAG_LEVELMULTIPLIER,
			X_MULTIPLIERTBOX, Y_MULTIPLIERTBOX, CX_MULTIPLIERTBOX, CY_MULTIPLIERTBOX,
		...
		case TAG_WEATHER:
		{
			...
			if (tag == TAG_OK || tag == TAG_OK_WHOLELEVEL)
			{
				if (ModifyHold())
				{
					...
					this->pRoomWidget->LoadFromRoom(this->pSelectedRoom, &this->LevelEntrances);
				}
			}
			Paint();
		}
	...
	switch (Key.keysym.sym)
	{
		...
		case SDLK_F2:
			...
			if (this->pSelectedHold)
			{
				...
				SetCursor(CUR_Wait);
				g_pTheDB->Holds.LogScriptVarRefs(this->pSelectedHold->dwHoldID);
				SetCursor();
			}
		break;
	...
	switch (dwTagNo)
	{
		...
		case TAG_STYLE_LBOX:
			if (!ModifyHold())
			{
				...
				const WSTRING newStyle = this->pStyleListBoxWidget->GetSelectedItemText();
		...
		//Whenever number changes, update the level data.
		case TAG_LEVELMULTIPLIER:
		{
			if (!ModifyHold())
			{
				WCHAR temp[12];
				_itoW(this->pSelectedLevel->dwMultiplier, temp, 10);
				this->pLevelMultiplier->SetText(temp);
				this->pLevelMultiplier->Paint();
			} else {
				const UINT newMultiplier = _Wtoi(this->pLevelMultiplier->GetText());
				if (newMultiplier)
				{
					this->pSelectedLevel->dwMultiplier = newMultiplier;
					this->pSelectedLevel->Update();
					this->pSelectedHold->Update();
				}
			}
...
//Set button states depending on what data are available.
{
	...
	static const UINT levelWidgetTag[wLevelWidgets] = {
		TAG_RENAME_LEVEL, TAG_REDESC_LEVEL, TAG_DELETE_LEVEL, TAG_LEVELMULTIPLIER
	};
	...
	//Room
	static const UINT wRoomWidgets = 3;
	static const UINT roomWidgetTag[wRoomWidgets] = {
		TAG_EDIT, TAG_ROOM_IS_SECRET, TAG_WEATHER //TAG_ROOM_IS_REQUIRED, 
	};
	for (wIndex=0; wIndex<wRoomWidgets; ++wIndex)
	{
		VERIFY(pButtons[wIndex] = GetWidget(roomWidgetTag[wIndex]));
		pButtons[wIndex]->Enable(this->pSelectedRoom != NULL);
	}
...
//Returns: new level's ID#
{
	...
	//Set default level item multiplier.
	pLevel->dwMultiplier = 1;
	...
	CEntranceData *pEntrance = new CEntranceData(0, 0, this->pSelectedRoom->dwRoomID,
			...
			SE, true, CEntranceData::DD_Always);
	...
	CDbRoom *pRoom)   //(in)
{
	...
	SelectListStyle(this->pSelectedRoom->style);
	CRoomScreen::SetMusicStyle(this->pSelectedRoom->style, SONG_PUZZLE);
...
//*****************************************************************************
void CEditSelectScreen::RotateLevelC()
//Rotates all rooms and room positions in the level 90 degrees about the center.
{
	if (!this->pSelectedHold || !this->pSelectedLevel)
		return;
	if (!ModifyHold())
	...
	for (CIDSet::const_iterator room=roomIDs.begin(); room!=roomIDs.end(); ++room)
	{
		CDbRoom *pRoom = g_pTheDB->Rooms.GetByID(*room);
		ASSERT(pRoom);
		pRoom->RotateClockwise();
		for (UINT wIndex=this->pSelectedHold->Entrances.size(); wIndex--; )
		{
			CEntranceData *pEntrance = this->pSelectedHold->Entrances[wIndex];
			if (pEntrance->dwRoomID == pRoom->dwRoomID)
				pEntrance->RotateClockwise(pRoom->wRoomCols);
		}
		const UINT wNewRoomX = 100 - (pRoom->dwRoomY % 100);
		pRoom->dwRoomY = (pRoom->dwRoomY/100)*100 + pRoom->dwRoomX;
		pRoom->dwRoomX = wNewRoomX;
		pRoom->Update();
		delete pRoom;
	}
	this->pSelectedHold->Update();
