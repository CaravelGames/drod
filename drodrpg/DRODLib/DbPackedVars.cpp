// $Id: DbPackedVars.cpp 9121 2008-08-05 04:07:15Z mrimer $
...
{
	const BYTE *pucRet = (BYTE*) GetVar(pszVarName, (void *)NULL); //bool is not always 1 byte, e.g. on Mac it's 4 bytes, so can't cast to a bool*
	if (pucRet)
	{
...
//*****************************************************************************
BYTE* CDbPackedVars::SetVar(const char *pszVarName, BYTE ucValue)
{
	return (BYTE *) SetVar(pszVarName, (const void *) &ucValue,
			1, //ensure we're only saving one byte on all OSes
			UVT_byte);
}
...
bool* CDbPackedVars::SetVar(const char *pszVarName, bool bValue)
{
	...
	return (bool*) SetVar(pszVarName, (const void *)&val,
			1, //ensure we're only saving one byte on all OSes
			UVT_bool);
}
