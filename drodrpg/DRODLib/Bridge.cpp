// $Id: Bridge.cpp 10156 2012-04-29 20:05:08Z TFMurphy $
...
// Should only be called by CDbRooms::InitRoomStats().
// When called with a coordinate that points
// t0o an already stored bridge component will duplicate it!
...
// is not a bridge
{
	...
	this->pRoom->GetConnectedTiles(wX, wY, this->bridgeMask, false, tiles, &this->ignoredTiles);
	if (tiles.empty())
...
//Drop the specified bridge.
{
	...
	//Exception: player can swim and bridge falls into the water.
	...
	if (player.IsInRoom() && tiles.has(player.wX, player.wY))
	{
		...
		{
			if (bIsPit(wTileToPlot) || !player.CanWalkOnWater())
...
//Returns: whether this tile supports a bridge
{
	...
	return !(bIsBridge(wTile) || bIsPit(wTile) || wTile == T_WATER || bIsPlatform(wTile));
}
